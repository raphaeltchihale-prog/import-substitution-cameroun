...
if selected == "Accueil":
    if not st.session_state.logged_in:
        ...
        # ------------------- ONGLETS ------------------- #
        tabs = st.tabs([
            "📊 Descriptif","🔁 Comparatif","📈 Dynamiques","🏆 Performance","📊 Tableau de Bord",
            "🔮 Prévisions","📝 Synthèse & Recommandations","📤 Export"
        ])
    ...
    # ================= PARTS PAR PRODUIT ================= #
      ...
      # ====== Fonction de prévision par moyenne mobile itérative ====== #
        def forecast_moving_average(series: pd.Series, window: int = 3, predict_periods: int = 5) -> Dict[int, float]:
            """
            Prévision par moyenne mobile itérative.
    
            series : pd.Series
                Série temporelle historique (ordonnée par année)
            window : int
                Taille de la fenêtre pour la moyenne mobile
            predict_periods : int
                Nombre d'années à prédire
            """
            series_clean = series.dropna().tolist()
            if len(series_clean) < window:
        # Trop peu de données, retourner NaN
               return {i: np.nan for i in range(predict_periods)}
            ...
            for _ in range(predict_periods):
                ...
                temp_series.append(ma)  # ajoute la prévision pour la prochaine itération

    # Les années futures
            last_year = series.index[-1]
            forecast_years = list(range(last_year + 1, last_year + 1 + predict_periods))

            return dict(zip(forecast_years, forecast_values))


# ====== Onglet Prévisions (moyenne mobile itérative) ====== #
        with tabs[5]:
            st.header("🔮 Prévisions simples (moyenne mobile itérative)")
            ...
            # Prévision
                ...
                fig.update_layout(
                    title=f"Prévisions pour {metric_prev} par moyenne mobile itérative",
                    xaxis_title="Année",
                    yaxis_title=metric_prev
                )
                st.plotly_chart(fig, use_container_width=True)
