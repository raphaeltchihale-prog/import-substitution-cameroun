...
for m in metrics:
    if m in df_f.columns:
        df_f[f"{m}Taux_de_croissance_importsubstitution%"] = df_f.groupby(col_produits)[m].pct_change(fill_method=None) * 100
...
with tabs[1]:
    ...
    if metric:
        ...
        Taux_de_croissance_importsubstitution = f"{metric}"
        if Taux_de_croissance_importsubstitution in df_f.columns:
            fig3 = px.line(df_f, x=col_annee, y=Taux_de_croissance_importsubstitution, color=col_produits, markers=True,
